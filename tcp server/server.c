...
#include <string.h>
#include <unistd.h>
#include <arpa/inet.h>  // inet_addr and htons() are defined in this header file
#include <sys/socket.h> // socket, bind, listen, accept, send, recv are defined in this
#include <sys/select.h> // select() and fd_set are defined in this header file
...
int main()
{
    ...
    while (1)
    {
        ...
        for (int i = 0; i < MAX_CLIENTS; i++)
        {
            ...
            if (sd > 0)
                FD_SET(sd, &readfds); // add clients to the set
            if (sd > maxfd)
                maxfd = sd;
        }
